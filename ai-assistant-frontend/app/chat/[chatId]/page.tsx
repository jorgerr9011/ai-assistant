...
export default function Chat() {
    ...
    const finish = async (message: Message) => {
        ...
        const resUpdate = await fetch(`http://localhost:3000/api/chat/${params.chatId}`, {
            method: 'GET',
            headers: {
        ...
        await fetch(`http://localhost:3000/api/chat/${params.chatId}`, {
            method: 'PUT',
            ...
            <div className='container items-end mb-40 h-2/5 overflow-y-auto'>
                {messages.map(m => (
                    <>
                        ...
                        {/*{m.role === 'user' ? (
                                <>
                                    <div className="flex flex-col items-end rounded overflow-y-auto">
                                        <div key={m.id} className="text-dark p-3 mx-4 mb-2 rounded-xl bg-gray-400">
                                            {m.role === 'user' ? 'User: ' : 'AI: '}
                                            {m.content}
                                        </div>
                ...
                {loading || isLoading ? (
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className="flex items-center px-2 py-2 bg-gray-50 rounded-lg dark:bg-gray-400">
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" onClick={handleDelete} fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-7 mr-2 ml-2 rounded-full cursor-pointer hover:bg-blue-100 dark:text-red-500 dark:hover:bg-gray-600">
                                <path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                            </svg>
