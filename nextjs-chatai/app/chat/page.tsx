...
export default function Chat() {
    ...
    const { setMessages, messages, input, handleInputChange, handleSubmit } = useChat({ onResponse: response, onFinish: finish });

    useEffect(() => {

        setMessages(usuario?.chat_history)
    }, [isLoading])

    return (
        <div className="flex flex-col mx-auto w-full h-5/6 max-h-full max-w-5xl py-12 rounded bg-gray-800">

            <div className="flex flex-col justify-start items-start h-max rounded overflow-y-auto">
                {messages.map(m => (
                    <div key={m.id} className="text-white mx-4 mb-2 rounded-md bg-gray-800">
                        {m.role === 'user' ? 'User: ' : 'AI: '}
                        {m.content}
                    </div>
                ))}
